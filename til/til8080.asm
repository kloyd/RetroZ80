...
;---------- Put in CP/M Transient Memory space.
 ...
 ENDIF
	JP	START
; Register Usage for Z80
; BC - Instruction Register 16 bit
; DE - Word Address and Scratch
; HL - Scratch 
; IX - Return Stack Pointer. How to do in 8080
; can it be a DW location ?
; IY - Address of NEXT
; Can JP (IY) be replaced with JP NEXT ?
...
ABORT	LXI		SP, STACK
		PUSH	D
		; This should be the 8080 way to store zero into the mode pointer variable. (8 bit var)
		LXI		H,0
		SHLD 	MODEPTR 
		; LD	(MODEPTR),HL
		; LD	IY,NEXT 
		; LD	IX,RETURN
		LXI		H, RETURN 
		SHLD	RS 		; Return Stack pointer.
		LXI		H,8080h
		SHLD	LBEND
		LXI		B, OUTER	; Effectively, Set OUTER as the next routine
...
NEXT	LDAX	B	;LD	A,(BC)	; BC = Instruction Register
		MOV		L, A 	;LD	L,A	; @I -> WA (HL = word address)
		INX 	B 
		LDAX	B 		;LD	A,(BC)
		MOV		H, A 	;LD	H,A
		INX		B 		;INC	BC	; I = I + 2
RUN		MOV		E, M 	;LD	E,(HL)	; @WA -> CA (Code Address)
		INX 	H 		;INC	HL	; WA = WA + 2
		MOV 	D, M	; LD	D,(HL)
		INX		H
		XCHG 		;EX	DE,HL	; CA -> PC
		PCHL 		; JP	(HL)

COLON	LHLD	RS		; DEC	IX
		DCX		H 		; Dec HL
		MOV	M, B		; LD (IX+0), B
		DCX		H 		;DEC	IX
		MOV M, C		;LD	(IX+0),C
		SHLD	RS 		; important the virtual register must always be saved!
		MOV 	C, E 	;LD	C,E
		MOV		B, D 	;LD	B,D
		JMP 	NEXT 	;JP	(IY)
;----------- End of Inner -----
...
; ORG	400h	; put on page boundary nope
